(function () {
  ...
  function toRoman (arabic) {
    ...
    if ('string' === typeof arabic || arabic instanceof String) {
      ...
      if (isNaN(arabic)) throw new TypeError('toRoman expects a number');
    }
    if ('number' !== typeof arabic) throw new TypeError('toRoman expects a number');

    // Rounding up "bad" numbers: NaN, negative numbers, numbers over 3999,...
    if (isNaN(arabic)) throw new TypeError('toRoman expects a real number');
